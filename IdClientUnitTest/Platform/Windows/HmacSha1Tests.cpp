...
UT_BeginTest(HmacSha1Base64Test)
{
	...
	uint8_t* bytes = HmacSha1(&destLen, (const uint8_t*)szData, strlen(szData), (const uint8_t*)szKey, strlen(szKey), &platformErrorCode);

	wchar_t* base64 = Base64Encode((const uint8_t*)bytes, destLen, &platformErrorCode);

	UT_AssertEquals(wcscmp(base64, L"xIcCRlnXa+IqFtO+9AF3OqeRdAU="), 0);

	FreeMemory((void**)&bytes);
}
UT_EndTest
